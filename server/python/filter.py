...
import util, pit, myldap, config # these modules shared between the client and server

HIGHSEC_SERVER='/var/spool/postfix/public/secure:-1'
LOWSEC_SERVER='localhost:25'

def filter__message(msg_file,sender,recipient):
    ...
    if m:
        ...
        try:
            hub = myldap.LDAP(True,config.password)
...
if _name__=='__main__':
    ...
    filter_message(sys.stdin,sys.argv[1],sys.argv[2])
